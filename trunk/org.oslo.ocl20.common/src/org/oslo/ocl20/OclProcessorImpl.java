...
abstract public class OclProcessorImpl implements OclProcessor {
     ...
     * */
    protected java.util.Set models = new HashSet();
    ...
    /**
     * Parse the passed string as an OCL expression
     * with output going to the passed log object.
     * 
     * A null valued result implies a successful (error free) parse.
     * 
     ...
     * 
     * The string may be a number of contexts.
     * The result contains a list of the values of the evaluated expression.
     * 
     */
     ...
     * 'context OclVoid inv:' is added to the front of the passed expression,
     * the type 'OclVoid' is added the the type environment,
    ...
    public List evaluate(String str) {
        ...
        while (i.hasNext()) {
            ...
            Map declRes = this.getEvaluator().evaluate(decl, runtimeEnvironment("self", this.getStdLibAdapter().Undefined()), log);
    ...
    /**
     * Evaluation of the passed string which contains a package declaration
     * in the context of self
     * with output going to System.out.
     * 
     * The string may be a number of contexts which will be evaluated using same runtime environment.
     * The result contains a list of the values of the evaluated expressions.
     * 
     */
    ...
    public List evaluate(String str, Object self, ILog log) {
        ...
        while (i.hasNext()) {
            ContextDeclaration decl = (ContextDeclaration) i.next();
            Map declRes = this.getEvaluator().evaluate(decl, runtimeEnvironment("self", self), log);
            result.addAll(declRes.values());
        }
        return result;
    }
    ...
    /**
     * Semantic analysis of the passed file which contains a package declaration
     * in the context of self
     * with output going to the passed log object.
     * 
     * The string may be a number of contexts which will be evaluated using same runtime environment.
    ...
    /**
     * Generate code for constraints passed as a file 
     * with provided indentation
     * and provided log
     * 
                ...
                if (cs != null) {
                    ...
                    while (i.hasNext()) {
                        ...
                        if (decl != null && decl instanceof ClassifierContextDecl) {
                            ...
                            //add Defined properties and Operations
                            Iterator ic = decl.getConstraint().iterator();
                            while (ic.hasNext()) {
                                Constraint cons = (Constraint) ic.next();
                                if (cons.getKind() == ConstraintKind$Class.DEF) {
                    ...
                    Iterator id = defs.keySet().iterator();
                    while (id.hasNext()) {
                        OclModelElementType selfType = (OclModelElementType) id.next();
                        List l = (List) defs.get(selfType);
                        Iterator ic = l.iterator();
                        while (ic.hasNext()) {
                            Constraint cons = (Constraint) ic.next();
                            if (cons.getDefProperty() != null) {
	                            String defName = cons.getDefProperty().getName();
    	                        Class retType = (Class) cons.getDefProperty().getType().getDelegate();
        	                    Class selfCls = (Class) selfType.getDelegate();
            	                OclAnyImpl result = (OclAnyImpl) this.getGenerator().generate(cons, tab + tab + tab + tab, this);
                	            output.println(tab + "public static " + retType.getName() + " " + defName + "(" + selfCls.getName() + " self) {");
                    	        output.println(tab + tab + "/*\n " + cons.accept(toStringVisitor, new HashMap()) + "\n*/");
                        	    output.print(result.getInitialisation());
                            	output.println(tab + tab + "return " + result.asJavaObject() + ";");
                            	output.println(tab + "}");
                            } else if (cons.getDefOperation() != null) {
								String defName = cons.getDefOperation().getName();
								Class retType = (Class) cons.getDefOperation().getReturnType().getDelegate();
								Class selfCls = (Class) selfType.getDelegate();
								OclAnyImpl result = (OclAnyImpl) this.getGenerator().generate(cons, tab + tab + tab + tab, this);
								output.println(tab + "public static " + retType.getName() + " " + defName + "(" + selfCls.getName() + " self");
								for(int pi = 0; pi < cons.getDefOperation().getParameterNames().size(); pi++) {
                    ...
                    output.println();
                    // Generate code for invariants
                    i = contexts.keySet().iterator();
                    while (i.hasNext()) {
                        OclModelElementType selfType = (OclModelElementType) i.next();
                        // Compute class name
                        ...
                        output.println(tab + "public static class " + clsName + " {");
                        ...
                        while (j.hasNext()) {
                            ...
                            while (k.hasNext()) {
                                ...
                                if (con.getKind() == ConstraintKind$Class.INV) {
                                    ...
                                    OclAnyImpl result = (OclAnyImpl) this.getGenerator().generate(con, tab + tab + tab + tab, this);
                                    Class resType = (Class) con.getBodyExpression().getType().getDelegate();
                                    // outputput the corresponding method
                                    output.println(tab + tab + "public static " + resType.getName() + " " + invName + "(" + selfTypeImplName + " self) {");
                                    output.println(tab + tab + tab + "/*\n " + con.accept(toStringVisitor, new HashMap()) + "\n*/");
                                    output.println(tab + tab + tab + "try {");
                                    output.print(result.getInitialisation());
                                    output.println(tab + tab + tab + tab + "return " + result.asJavaObject() + ";");
                                    output.println(tab + tab + tab + "} catch (Exception e) {");
                                    output.println(tab + tab + tab + tab + "return null;");
                                    output.println(tab + tab + tab + "}");
                                    output.println(tab + tab + "}");
                                }
                            }
                        ...
                        output.println(tab + tab + "public static java.util.Map evaluateAll(" + selfTypeImplName + " self) {");
                        output.println(tab + tab + tab + "java.util.Map result = new java.util.HashMap();");
                        j = invNames.iterator();
                        while (j.hasNext()) {
                            String invName = (String) j.next();
                            output.println(tab + tab + tab + "result.put(\"" + invName + "\"," + invName + "(self));");
                        }
    ...
    String defsMethod() {
        String s =
            "static public Object def(String propertyName, Object[] args) {\n"
